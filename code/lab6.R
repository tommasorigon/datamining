...
library(mgcv)
m_gam <- gam(Luxury ~ s(Overall.Qual) + s(Gr.Liv.Area) + s(House.Age) + s(Tot.Bathrooms), 
                data = ames_train, family = "binomial")
summary(m_gam)
plot(m_gam)
...
metrics(y = y_test, probs = y_hat_simple)
metrics(y = y_test, probs = y_hat_ridge)
metrics(y = y_test, probs = y_hat_lasso)
metrics(y = y_test, probs = y_hat_gam)
metrics(y = y_test, probs = y_hat_mars1)
metrics(y = y_test, probs = y_hat_mars2)
metrics(y = y_test, probs = y_hat_rf)
