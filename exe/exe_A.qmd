...
#### A.4 - Last step of the Gram-Schidmt algorithm

Show that the last statement of [this slide](../slides/un_A.html#the-qr-decomposition-and-least-squares) is correct, namely that the [last step]{.orange} of the [Gram-Schmidt]{.blue} algorithm
$$
\hat{\beta}_p = (\tilde{\bm{z}}_p^T\bm{y}) / (\tilde{\bm{z}}_p^T \tilde{\bm{z}}_p).
$$
coincides with the OLS estimate of the $p$th coefficient. 
...
A detailed description of this procedure ([Algorithm 2.2]{.blue} of the textbook Azzalini \& Scarpa, 2011) is given below:
...
Implement a function `logistic_mle(X, y)` which computes the maximum likelihood estimate for a logistic regression model using the [iteratively re-weighted least squares]{.blue}, as described [here](../slides/un_A.html#iteratively-re-weighted-least-squares-ii). 

Verify that the output of `logistic_mle` and the built-in `glm` function coincide on the [hearth dataset](../slides/un_A.html#the-heart-dataset).

#### A.8 - Additional exercises

Solve the exercises 2.1 - 2.8 of the [textbook]{.orange} Azzalini \& Scarpa (2011).