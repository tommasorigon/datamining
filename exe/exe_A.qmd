...
I have read, on a popular social media, a confusing sentence about standardization, that can be loosely translated as follows:
...
There are multiple mistakes within this single sentence. For instance, linear models do [not]{.orange} assume the Gaussianity of the input variables $x_{ij}$. Sometimes (but not necessarily!) the errors $\epsilon_1,\dots,\epsilon_n$ are assumed to be Gaussian.

But there is an even more striking mistake. Suppose the original input values $x_{ij}$ are standardized, namely the transformed variables $z_{ij}$ are such that 
$$
...
There are good reasons to standardize the variables, for instance, in ridge/lasso regression models. Moreover, centering the predictors may be helpful to get a design matrix which is better conditioned... But these motivations are not related to Gaussianity nor mentioned in the original post. 
