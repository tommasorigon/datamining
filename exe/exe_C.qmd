...
Consider a regression problem with $k$ variables having mean zero and standard deviation one. The response is also centered. Suppose also that each variable has [identical absolute covariance]{.blue} with the response:
$$
\text{cov}(\tilde{\bm{x}}_j, \bm{y}) = \frac{1}{n}\sum_{j=1}^k x_{ij} y_i= \lambda_{k-1}, \qquad j=1,\dots,k.
$$
Let $\hat{\beta}$ be the [least squares]{.blue} estimate and let us denote with $r_i(\alpha) = y_i - \alpha x_i^T\hat{\beta}$ the residuals from a fraction of the predictions, with $\alpha \in [0, 1]$.

[Prove]{.orange} that the following relationship holds:
$$
\text{cov}(\tilde{\bm{x}}_j, \bm{r}(\alpha)) = \frac{1}{n}\sum_{i=1}^nx_{ij}\{y_i - r_i(\alpha)\} = \lambda_{k-1} (1 - \alpha), \qquad j=1,\dots,k,
$$
implying that the magnitude of the residuals is constant across the covariates as we progress toward the least squares fit $x_i^T\hat{\beta}$.
...
#### C.8 - Support vector machine as a ridge minimization problem ☠️ 

Consider this exercise only if you already know  [Support Vector Machines]{.blue} (SVM). In SVM, we seek the optimal values $\hat{\beta}_0, \hat{\beta}$ that solve the following quadratic programming problem:
$$
...
$$
(\hat{\beta}_0,\hat{\beta}) = \arg\min_{\beta_0,\beta_1}\: \underbrace{\sum_{i=1}^n[1 - y_i(\beta_0 + \bm{x}_i^T\beta)]_+}_{\text{hinge loss}} + \underbrace{\lambda\sum_{j=1}^p\beta_j^2}_{\text{ridge penalty}},
$$
...
#### C.10 - Implementation of pathwise coordinate optimization

Implement the pathwise coordinate optimization algorithm that is described in [this slide](../slides/un_C.html#coordinate-descent). Use it to predict the `Salary` of the baseball players on the `Hitters` dataset.
