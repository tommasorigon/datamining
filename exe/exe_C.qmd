...
$$
where $C > 0$ is a cost parameter and $y_i\in\{-1, 1\}$ the [categorical response]{.orange}. The SVM classifier is then obtained as $\hat{G}(\bm{x}) = \text{sign}(\hat{f}(\bm{x}))$, with $\hat{f}(\bm{x}) = \hat{\beta}_0 + \bm{x}^T\hat{\beta}$.
