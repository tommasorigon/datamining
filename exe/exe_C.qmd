...
Consider a regression problem with $k$ variables $\bm{x}_j$ having mean zero and standard deviation one. The response variable is centered as well. Moreover, suppose that each variable has [identical absolute covariance]{.blue} with the response:
$$
