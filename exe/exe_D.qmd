...
What can be said about the Epanechnikov kernel? What can be said for the Epanechnikov kernel with an adaptive nearest-neighbor bandwidth $h(x)$?

#### Exercise D.2 - Local linear regression

Prove the Theorem of [this slide](../slides/un_D.html#local-linear-regression-ii) about the computationally efficient formula for local linear regression. 
...
$$
Conclude that if the points $(x_i, y_i)$ belong to a line, then the fitted values of a local linear regression coincide with $y_i$. 

Does the same property hold for the Nadaraya-Watson estimator? If yes, prove it. If not, show a counterexample.
...
[Prove]{.orange} the Theorem of [this slide](../slides/un_D.html#choice-of-the-bandwidth-ii), about the leave-one-out cross validation for linear smoothers. 
...
Write a function called `loclin(x, y)` that implements local linear regression using the formula of [this slide](../slides/un_D.html#local-linear-regression-ii). You can use any kernel function of your choice.

Compare the results of your function `loclin` on the `auto` dataset with those of the libraries `KernSmooth` and `sm`.
 
#### Exercise D.7 - Implementation of leave-one-out cross-validation

#### Exercise D.8 - The `trawl` dataset
