...
Prove the theorem about the explicit formula for [local linear regression estimator]{.blue} that is stated in [this slide](../slides/un_D.html#local-linear-regression-ii). 
...
[Hint]{.blue}. Start by showing that $\sum_{i=1}^ns_i(x) = 1$ and that $\sum_{i=1}^ns_i(x)(x_i - x) = 0$. Exploit these properties to obtain the proof. 
...
Then, show that the theorem is valid for the following estimators: 
  ...
  i. Nadaraya-Watson estimator. [Hint]{.blue}: sse the fact that Nadaraya-Watson is "projective" almost by definition.
  ...
  iv. Smoothing splines. [Hint]{.blue}: you may want to use a generalization of the Sherman-Morrison formula, called [Sherman–Morrison–Woodbury identity](https://en.wikipedia.org/wiki/Woodbury_matrix_identity). 
