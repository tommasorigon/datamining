...
Consider the [$k$-nearest neighbours]{.blue} estimator $\hat{f}(x)$ for a regression problem $Y_i = f(x_i) + \epsilon_i$, under the usual assumptions. 

[Find]{.orange} the bias, the variance, and effective degrees of freedom of $\hat{f}(x)$ and discuss their behavior as function of $k$. 
...
:::{.callout-note collapse=true}
#### Exercise D - Maximum likelihood estimate for $\lambda$
...
:::{.callout-note collapse=true}
#### Exercise E - General leave-one-out cross-validation 
...
:::{.callout-note collapse=true}
#### Exercise F - Truncated power basis

A cubic spline with one knot $\xi$ can be obtained using a basis of the form $x$, $x^2$, $x^3$ and ($x - \xi)_+$, as a consequence of the truncated power basis Theorem of [this slide](../slides/un_D.html#truncated-power-basis).
...
:::{.callout-note collapse=true}
#### Exercise G - Smoothing splines estimator
...
:::{.callout-note collapse=true}
#### Exercise H - Implementation of local linear regression
...
:::{.callout-note collapse=true}
#### Exercise I - Implementation of leave-one-out cross-validation
