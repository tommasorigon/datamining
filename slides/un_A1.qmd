...
-   $\bm{P}$ is a $n \times n$ [projection matrix]{.orange} matrix
    sometimes called [hat matrix]{.blue}. It can be shown that
    $\text{tr}(\bm{P}) = \text{rk}(\bm{P}) = p$. Moreover, it holds
    $\bm{P} = \bm{P}^T$ and $\bm{P}^2 = \bm{P}$.
...
-   However, the plot of the [residuals]{.orange}
    ...
    assumption, i.e. $\mathbb{E}(\epsilon_i) = \sigma^2$, might be
    violated.
...
-   This gives the statistician an lot of modelling flexibility, for
    instance:
...
-   This means that in [high-dimensional]{.orange} settings (large $p$)
    computations become rapidly [very costly]{.orange}, being cubic in
    $p$.
