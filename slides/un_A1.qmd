...
-   ... but you should be [very familiar]{.orange} with linear models
    and GLMs!
...
- ... and also that `fuel` has an non-negligible effect on the response.
...
-   To estimate the unknown parameters $\beta$, a possibility is to rely
    ...
    \ell(\beta) = \sum_{i=1}^n\{y_i - f(\bm{x}_i; \beta)\}^2,
    $$ using $n$ pairs of observations $(\bm{x}_i, y_i)$,
    for $i = 1,\dots,n$. 
...
-   The above specification is a special instance of [linear
    ...
    $$ where $\bm{x} = (1, x_1, \dots,x_{p-1})^T$ is a $p$-dimensional vector of [covariates]{.orange} and
    $\beta = (\beta_0,\dots,\beta_{p-1})^T$ is the corresponding vector of
    [coefficients]{.orange}.
:::

## Matrix notation

- It is often convenient to express the quantities of linear models in [matrix notation]{.orange}.

- The [response values]{.blue} are $\bm{y} = (y_1,\dots,y_n)^T$.

- The [design matrix]{.blue} is a $n \times p$ matrix, comprising the covariate's values, defined by
$$
...
$$
where $\bm{\epsilon} = (\epsilon_1,\dots,\epsilon_n)^T$ is a vector of iid error terms with zero mean and variance $\sigma^2$.

## Linear regression: estimation I

- The optimal set of coefficients $\hat{\beta}$ is the minimizer of the least squared criterion $$
\ell(\beta) = (\bm{y} - \bm{X}\beta)^T(\bm{y} - \bm{X}\beta) = ||\bm{y} - \bm{X}\beta||^2,
$$
where $||\bm{y}|| = \sqrt{y_1^2 + \cdots + y_n^2}$ is the [Euclidean norm]{.blue}. 

. . .

:::{.callout-note}
- If the design matrix has [full rank]{.blue}, that is if $\text{rk}(\bm{X}^T\bm{X}) = p$, then the [least square estimate]{.orange} has an explicit solution:
$$
...
$$
- $\bm{P}$ is a $n \times n$ matrix called [projection matrix]{.orange} such that $\text{tr}(\bm{P}) = \text{rk}(\bm{P}) = p$. It also holds that $\bm{P} = \bm{P}^T$ and $\bm{P}^2 = \bm{P}$.  
