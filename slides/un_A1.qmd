...
::: {.column width="60%"}
-   Consider `city.distance` ($y$), `engine.size`
    ($x$) and `fuel` ($z$).

-   The simplest model is a [linear
...
-   An estimate for the [unknown regression
    parameters]{.blue} $\beta_0$ and $\beta_1$ can be obtained by ordinary least squares...
...
-   The [optimal value]{.orange} is denoted by
    $\hat{\beta}$.
