...
## About this unit ([Homepage](../index.html))
...
-   We consider data for $n = 203$ models of cars in circulation in 1985
    in the USA.
-   We want to to [predict]{.blue} the distance covered per unit of
    fuel, as a function of the vehicle characteristics.
...
::: {.column width="60%"}
-   Consider `city.distance` ($y$), `engine.size` ($x$) and `fuel`
    ($z$).

-   The simplest model is a [linear regression]{.orange} line: $$
...
-   An estimate for the [unknown regression parameters]{.blue} $\beta_0$
    and $\beta_1$ can be obtained by ordinary least squares...
...
-   The [optimal value]{.orange} is denoted by $\hat{\beta}$.
...
-   The coefficient $R^2$ and the residual standard deviation $s$ are:
...
::: incremental
-   When the sample size $n$ is extremely large, as it is common in data
...
-   Using $\bm{W}$ and $\bm{u}$ one can obtain the least square estimate
...
-   We would like to [update]{.orange} the current least square estimate
