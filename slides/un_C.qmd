...
- Ridge regression is the most common [shrinkage method]{.blue}, in which the variability of the estimates is [bounded]{.orange}.
...
- If the [complexity parameter]{.blue} $s$ is large enough, then $\hat{\beta}_\text{ridge}$ is equivalent to 
 $\hat{\beta}$. 

- But when $s$ is small, the coefficients of $\hat{\beta}_\text{ridge}$ are explicitly [shrinked towards zero]{.blue}. Note that the intercept term $\beta_0$ is [not penalized]{.orange}. 

. . .

- Using the same [mathematical trick]{.blue} as [before](/#centering-the-predictors-i) (see Exercises), we can [omit the intercept]{.orange} term $\beta_0$ after [centering]{.orange} the inputs and the response, that is $\bar{y} = 0$ and $\bar{x}_{j} = 0$.

- The ridge solutions are [not equivariant]{.orange} under [scalings of the input]{.orange}, so one normally [standardizes]{.blue} the input if they are not in the same scale. 

## Lagrange multipliers and ridge solution

- An [equivalent formulation]{.blue}  can be obtained expressing the ridge problem in its [Lagrangian form]{.orange} ([Karush–Kuhn–Tucker multiplier method](https://en.wikipedia.org/wiki/Karush–Kuhn–Tucker_conditions)). The ridge estimator $\hat{\beta}_\text{ridge}$ is the minimizer of
$$
\sum_{i=1}^n(y_i - \bm{x}_i^T\beta)^2 + \lambda \sum_{j=1}^p\beta_j^2 = ||\bm{y} - \bm{X}\beta||^2 + \lambda||\beta||^2,
$$
for some [complexity parameter]{.orange} $\lambda > 0$, such that $s = ||\hat{\beta}_\text{ridge} ||^2$.

. . .

- When $\lambda = 0$ and $\bm{X}$ is full rank, then $\hat{\beta}_\text{ridge} = \hat{\beta}$. Conversely, when $\lambda \rightarrow \infty$ then $\hat{\beta}_\text{ridge} = 0$.
