...
- Principal components form an orthogonal basis of $\bm{X}$, but they are not a "random" choice and they do [not]{.orange} coincide with them [Gram-Schmidt]{.orange} basis of [Unit A.1](un_A1.html#the-qr-decomposition-i). 

- Indeed, the first principal component is the linear combination having [maximal variance]{.blue}:
$$
...
- Hence, the quantity $d_j^2 / \sum_{j'=1}^p d_{j'}^2$ is often used to measure the amount of total variance captured by principal components.  
