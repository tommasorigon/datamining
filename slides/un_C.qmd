...
- Thus, we let the [original data]{.blue} be $x_{ij}^*$ and $y_i^*$ and define their [transformation]{.orange}: $$
x_{ij} = x_{ij}^* - \bar{x}_j^*, \qquad y_i = y_i^* - \bar{y}^*, \qquad i=1,\dots,n.
$$
...
- In practice, we want to make predictions in the [original scale]{.blue}, which requires a simple [final adjustment]{.orange}. One simply compute the intercept term $$
\hat{\beta}_0 = \bar{y}^* - \bar{\bm{x}}^*\hat{\beta},
$$ 
and then make predictions via $\hat{\beta}_0 + (\bm{x}_i^*)^T\hat{\beta} = \bar{y} + \bm{x}_i^T\hat{\beta}$. 
...
From now on, we will assume the data have been previously [centered]{.orange}, namely $$
...
-   When [complexity parameter]{.blue} $s$ is small, the coefficients of
...
::: incremental
-   We can use the same [mathematical trick]{.blue} that we [exploited
    before](/#centering-the-predictors-i) to simplify computations.

-   Under centered predictors, one can show that
    $\hat{\beta}_0 = \bar{y}$ even in the ridge regression case. The
    proof is almost identical as before (see Exercises).

-   Hence, we can [omit the intercept]{.orange} term $\beta_0$ after
    [centering]{.orange} the inputs and the response: $$
...
-   Standardization is often times performed under the hood in most
...
-   The intercept term $\beta_0$ is [not penalized]{.orange}.

## Centering and scaling

-  Some considerations we discussed for ridge apply also here: in particular we assume that $\bm{y}$ and $\tilde{\bm{x}}_{j}$ are [centered]{.blue} and possibly [scaled]{.blue}. 
