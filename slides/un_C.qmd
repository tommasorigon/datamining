...
|:-----------|------:|-----------:|------:|------:|-----:|
|`(Intercept)` |  2.465|       2.477|  2.455|  2.458| 2.468|
|`lcavol`      |  0.680|       0.740|  0.287|  0.295| 0.532|
|`lweight`     |  0.263|       0.316|  0.339|  0.184| 0.169|
|`age`         | -0.141|       .|  0.056| -0.001| .|
|`lbph`        |  0.210|       .|  0.102|  0.129| .|
|`svi`         |  0.305|       .|  0.261|  0.187| 0.092|
|`lcp`         | -0.288|       .|  0.219|  0.070| .|
|`gleason`     | -0.021|       .| -0.016|  0.054| .|
|`pgg45`       |  0.267|       .|  0.062|  0.104| .|
...
- Best subset selection is appealing because of its [conceptual simplicity]{.blue}.
...
- Subset strategies tend to select models that are "[too simple]{.orange}", especially in presence of correlated variables. 

- Despite the recent advances, when $p$ is large best subset selection is [computationally unfeasible]{.orange}.
...
- In principal components regression, we replace [original data]{.orange} $Y_i = f(\bm{x}_i) + \epsilon_i$ with their [centered]{.blue} version: $$
x_{ij} - \bar{x}_j, \qquad y_i - \bar{y}, \qquad i=1,\dots,n; \ \ j=1,\dots,p.
$$

. . .

- In the end, we will make predictions in the [original scale]{.blue}, which requires a simple [final adjustment]{.orange}. One simply need to compute the intercept term $$
...
-   The [predicted values]{.orange} for the [centered responses]{.blue} $\bm{y}$ of the PCR with $k$ components are:
    $$
...
- If you transform back the coefficients, there is a [clean interpretation]{.blue} of the impact of the covariates on the response.

- Principal component regression can be easily generalized to the GLM case.

- Principal components might be interesting in their own right, as they describe the [dependence structure]{.blue} among covariates.
...
- [All the variables]{.orange} are used for predictions, which could be computationally demanding.
...
- Hence, in ridge regression we replace [original data]{.orange} $Y_i = f(\bm{x}_i) + \epsilon_i$ with their [standardized]{.blue} version: $$ 
...
- The above formula highlights that ridge will be very [effective]{.blue} in presence highly [correlated variables]{.blue}, as they will be "shrunk" away by the penalty. 

- What typically happens is that such a reduction in variance [compensate]{.orange} the increase in bias, especially when $p$ is large relative to $n$. 
...
-   [Minimizing]{.blue} the loss $||\bm{y} - \bm{X}\hat{\beta}_\text{ridge}||^2$
    over $\lambda$ is a [bad idea]{.orange}, because it would always
...
-   Let us recall that the original data are $Y_i = f(\bm{x}_i) + \epsilon_i$ and that the [optimism]{.orange} for a generic estimator
...
- Different R packages have [different defaults]{.blue} about other aspects too.

- For instance, the R package `glmnet` uses $\tilde{\lambda}$ and also [standardizes the response]{.blue} $\bm{y}$ and then
...
- In ridge regression, [all variables]{.orange} are used. This is in contrast with best subset selection.
:::
...
-   The intercept term $\beta_0$ is [not penalized]{.orange}, as for ridge, because we can remove it by centering the predictors.
...
- The above solution can be written as $\hat{\beta}_\text{lasso} = \mathcal{S}_\lambda(\hat{\beta}_\text{ols})$, where $\mathcal{S}_\lambda(x) = \text{sign}(x)(|x| - \lambda)_+$ is the [soft-thresholding]{.blue} operator and $(\cdot)_+$ is the [positive part]{.orange} of a number (`pmax(0, x)`).
...
- The single predictor special cases provides further intuition of why the lasso perform [variable selection]{.blue} and [shrinkage]{.orange}.


. . .

- Ridge regression induce shrinkage in a [multiplicative]{.orange} fashion and the regression coefficients reach zero as $\lambda \rightarrow \infty$.

- Conversely, lasso shrink the ordinary least squares in an [additive]{.blue} manner, [truncating]{.orange} them at [zero]{.orange} after a certain threshold. 

. . .

- Even though we do not have a closed-form expression for the lasso solution $\hat{\beta}_\text{lasso}$ when the covariates $p > 1$, the main intuition is preserved: lasso induces [sparsity]{.orange}!
...
-   As for ridge regression, the values of $\lambda$ are expressed in somewhat [arbitrary scale]{.orange}. 

- There exist a [maximum value]{.blue} $\lambda_\text{max}$, after which all the coefficients are zero:
$$
\lambda_\text{max} = \max_{j}\left| \text{cov}(\tilde{\bm{x}}_j, \bm{y})\right| = \max_{j}\left| \frac{1}{n}\sum_{i=1}^n x_{ij} y_i\right|.
$$

. . .

-   It is confirmed that the variable `lcavol` is arguably the most important, followed by `lweight` and `svi`, being the last variables to be set equal to zero.

. . .

-   The coefficient of `age`, `gleason` and `lcp` are negative and then are set equal to zero for higher values of $\lambda$. Compare this with the [ridge path]{.orange}.

## Least angle regression I

-   Least angle regression (LAR) is a "[democratic]{.orange}" version of [forward stepwise regression]{.blue}.

. . .

- Forward stepwise builds a model [sequentially]{.blue}, adding one variable at a time. At each step, the best variable is included in the [active set]{.orange} and then the least square fit is updated.

- LAR uses a similar strategy, but any new variable contributes to the predictions only "as much" as it deserves. 

. . .

- The origin of the idea is due to a tight connection between with [boosting]{.orange}...
...
- The LAR algorithm provides a way to [compute]{.blue} the entire [lasso path]{.orange} efficiently at the cost of a full least-squares fit.

- LAR sheds light on important [statistical aspects]{.orange} of the lasso. A nice [LAR - lasso - boosting]{.blue} connection is established, which is computationally and conceptually useful. 
:::
