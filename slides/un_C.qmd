...
In principal components regression, we assume the data have been previously [centered]{.orange}: $$
    \frac{1}{n}\sum_{i=1}^ny_i = 0, \qquad \frac{1}{n}\sum_{i=1}^nx_{ij} = 0, \qquad j=1,\dots,p.
    $$
:::

. . . 

- The above assumption means that the [original data]{.blue} $x_{ij}^*$ and $y_i^*$ have been [transformed]{.orange}: $$
x_{ij} = x_{ij}^* - \bar{x}_j^*, \qquad y_i = y_i^* - \bar{y}^*, \qquad i=1,\dots,n; \ \ j=1,\dots,p.
$$
By construction, the predictors $\tilde{\bm{x}}_{j}$ and the response $\bm{y}$ are [centered]{.orange}.
...
$$ 
and then compute the predictions via the formula $\hat{\beta}_0 + (\bm{x}_i^*)^T\hat{\beta} = \bar{y} + \bm{x}_i^T\hat{\beta}$. 

. . .

- [Remark]{.orange}. The centering operation is a mathematical trick that facilitate the exposition, but is unconsequential from a predictive point of view. 
...
-   The [variance]{.orange} of $\hat{\beta}_\text{pcr}$, assuming iid
    errors $\epsilon_i$ and after some algebraic manipulation, results:
    $$
...
-   The ridge estimator $\hat{\beta}_\text{ridge}$ is the most common
    [shrinkage method]{.blue} and is the [minimizer]{.orange} of $$
    \sum_{i=1}^n(y_i - \bm{x}_i^T\beta)^2 \qquad \text{subject to} \qquad \sum_{j=1}^p \beta_j^2 \le s.
    $$

. . .

-   When the [complexity parameter]{.blue} $s$ is small, the coefficients of
    $\hat{\beta}_\text{ridge}$ are explicitly [shrinked]{.orange}, i.e.
    biased, [towards zero]{.orange}.

-   On the other hand, if $s$ is large enough, then
    $\hat{\beta}_\text{ridge}$ is equivalent to
    $\hat{\beta}_\text{ols}$.
...
-   The intercept term $\beta_0$ is [omitted]{.blue} in the above equation and therefore is not penalized, because the predictors and the response have been [centered]{.orange}. 
...
-   The ridge regression problem can be [equivalently
    ...
    \sum_{i=1}^n(y_i - \bm{x}_i^T\beta)^2 + \lambda \sum_{j=1}^p\beta_j^2 = \underbrace{||\bm{y} - \bm{X}\beta||^2}_{\text{least squares}} + \underbrace{\lambda ||\beta||^2}_{\text{\textcolor{red}{ridge penalty}}},
...
- In ridge regression we center the original data, which implicitely means that the [intercept]{.blue} term is [not penalized]{.blue}.

- As for ordinary least squares, the centering operation can be regarded as [two-step estimation procedure]{.orange}. Refer to the Exercises for more insights.
...
For ridge problems we will assume the data have been previously [standardized]{.orange}, namely $$
    \frac{1}{n}\sum_{i=1}^ny_i = 0, \qquad \frac{1}{n}\sum_{i=1}^nx_{ij} = 0, \qquad \frac{1}{n}\sum_{i=1}^n x_i^2 = 1\qquad j=1,\dots,p.
    $$
:::
...
- In other words, the [original data]{.blue} $x_{ij}^*$ and $y_i^*$ are typically [standardized]{.orange}, obtaining: $$
x_{ij} = \frac{x_{ij}^* - \bar{x}_j^*}{s_j}, \qquad y_i = y_i^* - \bar{y}^*, \qquad i=1,\dots,n; \ \ j=1,\dots,p.
$$
where $s_j$ is the [standard deviation]{.blue} or some other [scaling factor]{.blue}.


- It is easy to show (see Exercises) that the [predictions]{.orange} in the [original scale]{.blue} are $$
\underbrace{\hat{\beta}_0 + (\bm{x}_i^*)^T \hat{\beta}_\text{ridge}^*}_{\text{predictions}}, \quad  \underbrace{\hat{\beta}_\text{ridge}^* = \text{diag}(1 / s_1,\dots, 1/s_p) \hat{\beta}_\text{ridge}}_{\text{scaled coefficients}}, \quad \underbrace{\hat{\beta}_0 = \bar{y}^* - \bar{\bm{x}}^*\hat{\beta}_\text{ridge}^*}_{\text{intercept}}.
$$

. . .

- Different R packages have [different defaults]{.blue}. For instance
    `glmnet` also [standardizes the response]{.blue} $y$ and then
...
-   The [variance]{.orange} of $\hat{\beta}_\text{ridge}$, assuming iid
    errors $\epsilon_i$ with variance $\sigma^2$, results: $$
...
Let us [additionally]{.blue} assume that
$y_i = \bm{x}_i^T\beta + \epsilon_i$, where $\epsilon_i$ are iid errors
with variance $\sigma^2$. Then, there exist a $\lambda > 0$ such that $$
...
-   [Minimizing]{.blue} $||\bm{y} - \bm{X}\hat{\beta}_\text{ridge}||^2$
...
-   After some algebra, one finds that the effective degrees of freedom
    ...
    $$ where
    $\bm{H}_\text{ridge} = \bm{X}(\bm{X}^T\bm{X} + \lambda I_p)^{-1}\bm{X}^T$
    is the [hat matrix]{.orange} of ridge regression.
...
-   Using the above result, we can [plug-in]{.orange}
    ...
    $\hat{\sigma}^2 = (n - \text{df}_\text{ridge})^{-1} \sum_{i=1}^n(y_i - \bm{x}_i^T\hat{\beta}_\text{ridge})^2$.
:::
...
-   The lasso estimator $\hat{\beta}_\text{lasso}$ is the [minimizer]{.orange} of $$
...
-   The intercept term $\beta_0$ is [omitted]{.blue} in the above equation and therefore is not penalized, because the predictors and response have been [centered]{.orange}. 
...
- The above assumption means that the [original data]{.blue} $x_{ij}^*$ and $y_i^*$ have been [standardized]{.orange}: $$
x_{ij} = \frac{x_{ij}^* - \bar{x}_j^*}{s_j}, \qquad y_i = y_i^* - \bar{y}^*, \qquad i=1,\dots,n; \ \ j=1,\dots,p,
$$
where $s_j^2 = n^{-1}\sum_{i=1}^n (x_{ij}^* - \bar{x}^*_j)^2$ is the sample variance.

. . .

- It is easy to show (see Exercises) that the [predictions]{.orange} in the [original scale]{.blue} are $$
\underbrace{\hat{\beta}_0 + (\bm{x}_i^*)^T \hat{\beta}_\text{lasso}^*}_{\text{predictions}}, \quad  \underbrace{\hat{\beta}_\text{lasso}^* = \text{diag}(1 / s_1,\dots, 1/s_p) \hat{\beta}_\text{lasso}}_{\text{scaled coefficients}}, \quad \underbrace{\hat{\beta}_0 = \bar{y}^* - \bar{\bm{x}}^*\hat{\beta}_\text{lasso}^*}_{\text{intercept}}.
$$
...
- Having remove the intercept, the lasso estimator $\hat{\beta}_\text{lasso}$ is the [minimizer]{.orange} of $$
    \underbrace{\textcolor{darkblue}{\frac{1}{2 n}}\sum_{i=1}^n(y_i - \bm{x}_i^T\beta)^2}_{\text{least squares}} + \underbrace{\lambda \sum_{j=1}^p|\beta_j|}_{\text{\textcolor{red}{lasso penalty}}}
    $$ where $\lambda > 0$ is a [complexity parameter]{.blue}
...
- To gain some understanding, let us consider the [single-predictor]{.blue} scenario, in which$$
   \hat{\beta}_\text{lasso} = \arg\min_{\beta}\sum_{i=1}^n(y_i - x_i\beta)^2 + \lambda |\beta|.
    $$
...
$$
\hat{\beta}_\text{ridge} = \frac{1}{\lambda + 1}\text{cov}(x,y) =\frac{1}{\lambda + 1}\hat{\beta}_\text{ols}.
$$
:::
