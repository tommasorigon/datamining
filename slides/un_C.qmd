...
- Another nice property, shared by ridge, is that whenever $\tilde{\bm{x}}_j = \tilde{\bm{x}}_\ell$, then $\hat{\beta}_{j,\text{en}} = \hat{\beta}_{\ell,\text{en}}$. On the other hand, the lasso estimator would be undefined.
...
$$
We [cycle]{.orange} this update for $k=1,\dots,p$, again and again, until convergence.
...
- The coordinate descent algorithm is implemented in the `glmnet` package. It is, de facto, the main algorithm for penalized linear models. 
...
- The `glmnet` implementation is very efficient due to several additional tricks:
  ...
  - Active set convergence... + many other additional tricks. 
...
- Although the whole unit was devoted to [regression]{.orange} problems, essentially all these ideas can be [extended to GLMs]{.blue}. 
...
- The shrinkage effect and their ability to control the variance remain unaltered, but the theory holds only in an approximate sense. 
:::
