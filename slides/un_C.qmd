...
- The values of $\lambda$ are in somewhat [arbitrary scale]{.orange}. The ridge penalty has a concrete effect starting from $\lambda > 10$ or so.
...
- Most statistical software use a slightly [different parametrization]{.orange} for $\lambda$, as they  minimize $$
\textcolor{red}{\frac{1}{n}}\sum_{i=1}^n(y_i - \bm{x}_i^T\beta)^2 + \tilde{\lambda} \sum_{j=1}^p\beta_j^2,
$$
where $\tilde{\lambda} = \lambda / n$.

- This parametrization is more amenable for cross-validation as the new penalty $\tilde{\lambda}$ does not depend on the sample size. The R package `glmnet` uses this convention.

- This is mostly a cosmetic variant, but it might have [practical consequences]{.orange} if the selected $\lambda$ were identified using a fraction of the data and then the same $\lambda$ were used on larger dataset.

- Conversely, the parameter $\tilde{\lambda}$ is not affected by this behavior.
...
## A comparison between ridge, lasso and elastic net
