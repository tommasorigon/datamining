...
-   In this unit the [intercept]{.blue} $\beta_0$ will play a special
...
-   You should already know PCA as a tool for explorative analysis... We
    will focus on its usage for regression purposes.
:::
:::
...
-   There is a computational advantage of this reparametrization: the estimates
    for $(\alpha, \beta)$ can be computed separately and [in two steps]{.orange}.
...
For the rest of this Unit, we will assume the data have been previously [centered]{.orange}, namely $$
...
-   Using centered predictor means that we can focus on linear
    ...
    $$
whose usefulness will become clear shortly. 


## Centering the predictors III

-   Under such the centering assumption the [covariance matrix]{.orange} of the
    data is simply $$
...
- [Remark]{.orange}. The centering operation is a mathematical trick that will facilitate computations, but is unconsequential from a predictive point of view. 
