...
-   Best subset and forward regression can be used, at least in
    principle, even when $p > n$.
:::
...
-   Principal component regression can be easily generalized to the GLM
    case.
...
-   When $\lambda = 0$ the penalty term disappears and
    ...
    hand, there exists a finite value of $\lambda_\text{max} < \infty$
    such that $\hat{\beta}_\text{lasso} = 0$.

-   For any intermediate value $0 < \lambda < \lambda_\text{max}$ we get
...
<!-- -   There exist a [maximum value]{.blue} $\lambda_\text{max}$, after -->
...
- By construction, the coefficients in LAR change in a [piecewise]{.orange} fashion, with knots in $\lambda_k$. The LAR path coincides [almost always]{.blue} with the lasso. If not, a simple modification is:
...
- More formally, suppose $\bm{X}$ has [full rank]{.orange} $\text{rk}(\bm{X}) = p$ and $\bm{y}$ follows a Gaussian law. Then:
$$
...
- Generalizations of these results to the $p > n$ case are possible. 
...
#| fig-height: 5
#| fig-align: center
ggplot(data = data_df, aes(x = active_set, y = df, col = Method)) + geom_point() +
  geom_line() +
  theme_light() +
  theme(legend.position = "top") +
  scale_color_tableau(palette = "Color Blind") +
...
-   LAR and Lasso are extremely efficient approaches that perform both [variable selection]{.blue} and [shrinkage]{.orange} at the same time. It produces a [parsimonious]{.blue} model.

- [Bayesian interpretation]{.blue}: the penalty can be interpreted as a Laplace prior on $\beta$.
:::
...
tab_results <- matrix(c(mean((y_test - pred_ols)^2),
            mean((y_test - pred_best)^2),
            mean((y_test - pred_pcr)^2),
            mean((y_test - pred_ridge)^2),
            mean((y_test - pred_lasso)^2)), nrow = 1)
...
$$
    \frac{1}{2n}\sum_{i=1}^n(y_i - \bm{x}_i^T\beta)^2 + \lambda\left(\alpha \sum_{j=1}^p |\beta_j| + (1 - \alpha)\sum_{j=1}^p\beta_j^2\right),
$$ 
where $0 \le \alpha \le 1$ and the [complexity parameter]{.orange} $\lambda > 0$.

. . .

- [Ridge regression]{.orange} is a special case, when $\alpha = 0$. [Lasso]{.blue} is also a special case, when $\alpha = 1$.

- It is often not worthwhile to estimate $\alpha$ using cross-validation. A typical choice is $\alpha = 0.5$. 
...
-   In the [single-predictor]{.blue} scenario the elastic-net minimization problem simplifies to $$
     \hat{\beta}_\text{en} = \arg\min_{\beta}\sum_{i=1}^n(y_{i} - x_{i}\beta)^2 + \lambda \left( \alpha |\beta| + (1-\alpha) \beta^2\right).
      $$
...
-   It can be shown that an [explicit expression]{.orange} for $\hat{\beta}_\text{en}$ is available, which is 
    ...
    where $\mathcal{S}_\lambda(x) = \text{sign}(x)(|x| - \lambda)_+$ is the [soft-thresholding]{.blue} operator.

. . .

-   This clarifies that elastic-net is a compromise between lasso and ridge. However, the most important consequence of this finding is on [computations]{.orange}.
