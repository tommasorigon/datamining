...
-   Best subset and forward regression can be used, if computationally feasible, even when $p > n$.
:::
...
-   In the end, we will make predictions in the [original scale]{.blue},
    ...
    $\hat{\beta}_0 + \bm{x}_i^T\hat{\beta} = \hat{\alpha} + (\bm{x}_{i}^T - \bar{\bm{x}})\hat{\beta}$.

. . .

-   [Remark]{.orange}. The centering operation is a mathematical trick
    that simplifies the derivation but is inconsequential from an
...
-   In case of [multicollinearity]{.blue}, then the [last]{.orange}
    principal components will have a small variance, i.e., a small $d_j^2$.
...
-   When the [complexity parameter]{.blue} $s$ is small, the
    coefficients are explicitly [shrunk]{.orange}, i.e. biased,
    [towards zero]{.orange}.
...
ggplot() +
  geom_circle(aes(x0 = 0, y0 = 0, r = 2), color = "#FF8C00", fill = "#FF8C00") +
  geom_ellipse(aes(x0 = x0, y0 = y0, a = a, b = b, angle = angle), color = "#3232AA", data = ellipses) +
  geom_point(aes(x = x, y = y), data = points, color = "#3232AA") +
  theme_minimal() +
  ...
  coord_fixed() +
  annotate("text", x = 5.4, y = 3.2, label = expression(hat(beta)[ols]), col = "#3232AA") +
  annotate("text", x = -1, y = 2.5, label = expression(beta[1]^2 + beta[2]^2 == s), col = "#FF8C00") +
  annotate("text", x = 1.7, y = 4.8, label = expression(sum((y[i] - x[i]^T * beta)^2, i == 1, n) == c), col = "#3232AA")
```
...
-   The [lasso]{.blue} was introduced in the highly influential paper of Tibshirani (1996). It is a method that performs both [shrinkage]{.blue} and [variable selection]{.orange}
...
-   The lasso estimator is the [minimizer]{.orange} of the following
    ...
    [shrunk]{.orange} and when $s$ is large enough
...
-   For any intermediate value $0 < \lambda < \lambda_0$ we get
    a combination of [shrunk]{.orange} but positive coefficients, and
...
ggplot() +
  geom_rhombus(color = "#FF8C00", fill = "#FF8C00") +
  geom_ellipse(aes(x0 = x0, y0 = y0, a = a, b = b, angle = angle), color = "#3232AA", data = ellipses) +
  geom_point(aes(x = x, y = y), data = points, color = "#3232AA") +
  theme_minimal() +
  geom_vline(xintercept = 0, linetype = "dotted") +
  ...
  ylab(expression(beta[2])) +
  coord_fixed() +
  annotate("text", x = 2.9, y = 2.9, label = expression(hat(beta)[ols]), col = "#3232AA") +
  annotate("text", x = 2, y = -0.5, label = expression(paste("|", beta[1], "|") + paste("|", beta[2], "|") == s), col = "#FF8C00") +
  annotate("text", x = 5, y = 0.7, label = expression(sum((y[i] - x[i]^T * beta)^2, i == 1, n) == c), col = "#3232AA")
```
...
-   For [ridge regression]{.orange} (including a $n^{-1}$ factor in the least squares penalty; see [here](#elastic-net-with-a-single-predictor)) we get: $$
...
-   The LAR algorithm provides a way to [compute]{.blue} the entire
    [lasso path]{.orange} efficiently at the computational cost of a single
    least-squares fit.
...
## Effective degrees of freedom of LAR and best subset
...
- The interested reader may have a look at the (very technical) Chapter 11 of Hastie, Tibshirani and Wainwright (2015)
...
$$
\frac{1}{2n}\sum_{i=1}^n \left(\textcolor{red}{y_i - \sum_{k \neq j} x_{ik}\beta_k} - x_{ij} \beta_j\right)^2 + \lambda\left(\alpha  |\beta_j| + \frac{(1 - \alpha)}{2} \beta_j^2\right) + \underbrace{\lambda  \sum_{k\neq j}^p \{\alpha |\beta_k| + \frac{(1 - \alpha)}{2}\beta_k^2\}}_{\text{does not depend on } \beta_j},
$$ 

. . .

- Let us define the [partial residuals]{.blue} $r_i^{(j)} = y_i - \sum_{k \neq j} x_{ik}\beta_k$. Then the updated $\beta_j$ is
$$
\beta_j \leftarrow \frac{1}{1 + (1 - \alpha)\lambda}\mathcal{S}_{\alpha\lambda}\left(\frac{1}{n}\sum_{i=1}^n x_{ij} r_i^{(j)}\right).
$$
We [cycle]{.orange} this update for $j=1,\dots,p$, over and over until convergence.
...
- The `glmnet` implementation is very efficient due to several additional tricks:
  - The [warm start]{.blue}. The algorithm for $\lambda_k$ is initialized at the previous solution using $\lambda_{k-1}$.
  - Partial residuals can be efficiently obtained [without re-computing]{.orange} the whole linear predictor. 
  - The code is written in [C++]{.blue} from  [version 4.1-3](https://cran.r-project.org/web/packages/glmnet/news/news.html), it used to be written in [Fortran]{.orange}.
...
- However, computations are much harder: leaps-and-bound approaches can not be applied.
:::
