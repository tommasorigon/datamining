...
::: incremental
-   Let us consider a [reparametrization]{.blue} of the linear model
    using [centered predictors]{.orange} $$
    \begin{aligned}
...
-   In this reparametrization the centered predictors are
    [orthogonal]{.blue} to the [intercept]{.blue}. Thus, the estimates
    for $(\alpha, \beta)$ can be computed [in two steps]{.orange}.
...
-   Then, the [estimate of $\beta$]{.blue} can be obtained considering a
    linear model [without intercept]{.blue}: $$
...
::: incremental
-   From here on, and without loss of generality, we assume that
    everything is [centered]{.orange}: $$
    \sum_{i=1}^ny_i = 0, \qquad \sum_{i=1}^nx_{ij} = 0, \qquad j=1,\dots,p.
    $$

-   Hence, because of the previous results, we can focus on linear
    models [without intercept]{.blue}: $$
    f(\bm{x}_i; \beta) = x_{i1}\beta_1 + \cdots + x_{ip}\beta_p = \bm{x}_i^T\beta.
    $$

-   Under such an assumption the [covariance matrix]{.orange} of the
    data is simply $$
...
-   The intercept term $\beta_0$ is [not penalized]{.orange}. The considerations and same mathematical tricks discussed for ridge apply here. We assume that $\bm{y}$ and $\tilde{\bm{x}}_{j}$ are [centered]{.blue} and possibly [scaled]{.blue}. 
...
- Having remove the intercept, the lasso estimator $\hat{\beta}_\text{lasso}$ is the [minimizer]{.orange} of $$
    \underbrace{\sum_{i=1}^n(y_i - \bm{x}_i^T\beta)^2}_{\text{least squares}} + \underbrace{\lambda \sum_{j=1}^p|\beta_j|}_{\text{\textcolor{red}{lasso penalty}}}
    $$ where $\lambda > 0$ is a [complexity parameter]{.blue}
...
- Let us consider the [single-predictor]{.blue} scenario, in which$$
   \hat{\beta}_\text{lasso} = \arg\min_{\beta}\sum_{i=1}^n(y_i - x_i\beta)^2 + \lambda |\beta|.
    $$
    
- This problem admits an [explicit expression]{.orange} (see Exercises), which is
$$
\hat{\beta}_\text{lasso} = \begin{cases}\sum_{i=1}^n x_i y_i - \lambda, \qquad &\text{if} \quad \sum_{i=1}^n x_i y_i > \lambda \\
0 \qquad &\text{if} \quad \sum_{i=1}^n x_i y_i \le \lambda\\
\sum_{i=1}^n x_i y_i + \lambda, \qquad &\text{if} \quad \sum_{i=1}^n x_i y_i < -\lambda \\
\end{cases}
$$

- The above solution can be written as $\hat{\beta}_\text{lasso} = \mathcal{S}_\lambda(\hat{\beta}_\text{ols})$, where $\mathcal{S}_\lambda(x) = \text{sign}(x)(|x| - \lambda)$ is the [soft-thresholding]{.blue} operator.

- Note that for [ridge regression]{.orange} we would get
$$
\hat{\beta}_\text{ridge} = \frac{1}{\lambda + 1}\sum_{i=1}^n x_i y_i =\frac{1}{\lambda + 1}\hat{\beta}_\text{ols}.
$$
:::
