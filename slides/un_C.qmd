...
-   In principal component regression and in other related methods
    ...
    [compress]{.orange} the [intercept]{.orange} term $\beta_0$. In a
    sense, we want to "remove it".
...
-   For fixed values of $\lambda$, [efficient computations]{.orange} are
    available using QR and Cholesky decomposition.
...
-   The [lasso]{.blue} is in the highly influential paper of Tibshirani
    (1996). It is a method that performs both [shrinkage]{.blue} and
    [variable selection]{.orange}
...
- By construction, the coefficients in LAR change in a [piecewise]{.orange} fashion, with knots in $\lambda_k$. The LAR path coincides [almost always]{.blue} with the lasso. If not, a simple modification is needed:
...
- The lasso can be computed even when $p > n$. In those cases, will it be [unique]{.orange}?
...
- Non-uniqueness occurs in presence of [discrete-valued]{.orange} data. It is of practical concern only whenever $p > n$ and if we are interested in interpreting the coefficients. 
...
- [Miraculously]{.orange}, one can show that for the lasso, with a fixed penalty parameter $\lambda$, the number of nonzero coefficients $|\mathcal{A}_\text{lasso}(\lambda)|$ is an [unbiased estimate]{.blue} of the degrees of freedom.
...
- The above relationship is [exact]{.blue} if we consider the [LAR active set]{.orange} and under further conditions, implicitly using a different set of $\lambda$ values for any fit:
$$
