...
$$ where the local coefficients are obtained from
    ...
    $$
The above minimization problem is an instance of [weighted least squares]{.orange} and its solution is well known, as we have seen in Unit A. In fact, it can be shown that $$
    \hat{\beta}(x) = (\bm{X}_x^T\bm{W}_x\bm{X}_x)^{-1}\bm{X}_x^T\bm{W}_x\bm{y},
    $$ where the rows of $\bm{X}_x$ are $\bm{x}_{i,x} = (1, x_i - x)$
and $\bm{W}_x = \text{diag}\{w_1(x),\dots,w_n(x)\}$. 
...
The local regression coefficients can be equivalently written as $\hat{\beta}(x) = (n^{-1} \bm{X}_x^T\bm{W}_x\bm{X}_x)^{-1}(n^{-1} \bm{X}_x^T\bm{W}_x\bm{y})$. Moreover, the inverse of $n^{-1}\bm{X}_x^T\bm{W}_x\bm{X}_x$ can be easily evaluated, giving:
$$
