...
-   To improve the fit, we could consider [piecewise polynomial]{.blue}
    functions rather than constant. For example, a piecewise [linear]{.orange}
    function with $K = 12$ is $$
    \begin{aligned}
    h_1(x) &= I(x < \xi_1), \qquad && h_2(x) = x\:I(x < \xi_1),  \\
    h_3(x) &= I(\xi_1 \le x  <\xi_2), \qquad &&h_4(x) = x\:I(\xi_1 \le x < \xi_2), \\
    \vdots & &&\vdots\\
    h_{11}(x) &= I(x \ge \xi_5), \qquad &&h_{12}(x) = x\:I(x \ge \xi_5). \\
    \end{aligned}
    $$
...
-   The piecewise linear function
    $f(x; \beta) = \sum_{j=1}^{12} h_j(x) \beta_j$ is [not continuous]{.orange} e.g. at the knot $\xi_1$: $$
    \beta_1 + \beta_2 \xi_1 \neq \beta_3 + \beta_4 \xi_1.
    $$ To achieve [smoothness]{.orange}, it would be appealing to add
    some [continuity constraints]{.blue}.
...
- Cubic splines are continuous functions such that $f(x, \beta)$ is a [cubic polynomial]{.orange} over each interval $(a, \xi_1), (\xi_1,\xi_2),\dots, (\xi_k, b)$ and $f(x, \beta)$ has continuous first and second derivative. 
:::
...
Let $\xi_1 < \cdots < \xi_k$ be a set of ordered points called [knots]{.orange} belonging to the interval $(a, b)$. Let $$
...
## Truncated power basis III

## Natural splines

- A cubic spline on an interval $[\xi_0, \xi_{k+1}]$ will be said to be a **natural cubic
spline** if its second and third derivatives are zero at $\xi_0$ and $\xi_{k+1}$.
