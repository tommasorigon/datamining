...
The local linear regression estimator $\hat{f}(x)$ coincides with
$$
    \hat{f}(x) = \hat{\beta}_1(x) + \hat{\beta}_2(x) (x - x) = \hat{\beta}_1(x),
    $$ where 
    $$
    ...
    $$
    The above minimization problem is called [weighted least squares]{.orange} and the solution is known, as we have seen in Unit A. In fact, we have that $$
    \hat{\beta}(x) = (\bm{X}_x^T\bm{W}_x\bm{X}_x)^{-1}\bm{X}_x^T\bm{W}_x\bm{y},
    $$ where the rows of $\bm{X}_x$ are $\bm{x}_{i,x} = (1, x_i - x)$
...
$$
Note that we can write  $\hat{\beta}(x) = (n^{-1} \bm{X}_x^T\bm{W}_x\bm{X}_x)^{-1}(n^{-1} \bm{X}_x^T\bm{W}_x\bm{y})$. Moreover, the inverse of $n^{-1}\bm{X}_x^T\bm{W}_x\bm{X}_x$ can be easily calculated, being a $2 \times 2$ matrix, obtaining:
$$
...
\end{aligned}
$$
which means that the first component $\hat{f}(x) = \hat{\beta}_1(x)$ of the above vector coincides with
$$
\begin{aligned}
