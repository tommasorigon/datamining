...
The local linear regression estimator $\hat{f}(x)$ is defined as $$
    \hat{f}(x) = \hat{\beta}_1(x) + \hat{\beta}_2(x) (x - x) = \hat{\beta}_1(x),
$$ where the local coefficients are obtained from $$
    ...
    $$ The above minimization problem is an instance of [weighted least
squares]{.orange} and its solution is well known, as we have seen in
Unit A. In fact, it can be shown that $$
    \hat{\beta}(x) = (\bm{X}_x^T\bm{W}_x\bm{X}_x)^{-1}\bm{X}_x^T\bm{W}_x\bm{y},
    $$ where the rows of $\bm{X}_x$ are $\bm{x}_{i,x} = (1, x_i - x)$
and $\bm{W}_x = \text{diag}\{w_1(x),\dots,w_n(x)\}$.
...
Recall that $$
a_j(x) = n^{-1}\sum_{i=1}^n w_i(x) (x_i - x)^j,$$ for $j=0,1,2$, then:
$$
...
The local regression coefficients can be equivalently written as
$\hat{\beta}(x) = (n^{-1} \bm{X}_x^T\bm{W}_x\bm{X}_x)^{-1}(n^{-1} \bm{X}_x^T\bm{W}_x\bm{y})$.
Moreover, the inverse of $n^{-1}\bm{X}_x^T\bm{W}_x\bm{X}_x$ can be
...
\end{aligned}
$$ which means that the first component $\hat{f}(x) = \hat{\beta}_1(x)$
...
-   In linear smoothers, we can express the predicted values
    ...
    \hat{\bm{y}} = \sum_{i=1}^n\tilde{\bm{s}}_i y_i = \bm{S}\bm{y}, \qquad \tilde{\bm{s}}_i = (s_i(x_1), \dots, s_i(x_n))^T,
    $$ where $\bm{S} = (\tilde{\bm{s}}_1,\dots,\tilde{\bm{s}}_n)$ is the so-called
...
$$ and $$
h_{j + d + 1}(x) = (x - \xi_j)_+^d, \qquad j = 1, \dots, k.
