...
-   To get an intuition of this, consider the following Taylor expansion
    ...
    & = f(x_0)\underbrace{\sum_{i=1}^ns_i(x_0)}_{=1} + f'(x_0) \underbrace{\sum_{i=1}^n(x_i - x)s_i(x_0)}_{=0} + \frac{f''(x_0)}{2}\sum_{i=1}^n(x_i - x_0)^2s_i(x_0) + \text{rest}.
    \end{aligned}
    $$
...
-   In a [generalized linear model]{.blue} with a single predictor, for
    ...
    (\hat{\beta}_1,\hat{\beta}_2)= \arg\max_{(\beta_1, \beta_2)}\sum_{i=1}^nw_i(x) \ell(\beta_1 + (x_i - x_0)\beta_2; y_i) ,
...
- Smoothing parameter can be easily handled, being a linear smoother.
:::
...
- For this reason, more [computationally convenient]{.blue} bases are preferred. This means we will consider an [equivalent]{.blue} set of functions $\mathcal{B}_1(x),\dots,\mathcal{B}_K(x)$ such that
$$
...
## ☠️ - Definition of B-splines

- Define $\xi = (\xi_1,\dots,x_k)$ and consider an [augmented]{.orange} sequence of ordered [knots]{.orange} $$
\tau = (\tau_1,\dots,\tau_{K + 2M}) = (\underbrace{\xi_{-M+1},\dots,\xi_0}_{\text{auxiliary knots}}, \: \xi,\: \underbrace{\xi_{k+1},\dots,\xi_{k + M}}_{\text{auxiliary knots}}).
$$
The most common choice is $\xi_{-M+1}= \dots = \xi_0 = a$ and $\xi_{k+1} = \dots = \xi_{k + M} = b$.

. . .

- [Step 1]{.blue}. For $j = 1,\dots, K + 2M - 1$, obtain the B-spline of order $m = 0$ as follows: 
$$
...
- [Step 2 (recursion)]{.orange}. The B-spline of [order $m \le M$]{.blue} are obtained [recursively]{.orange}, so that  $$
\mathcal{B}_{j, m}(x) = \frac{x - \tau_j}{\tau_{j + m - 1} - \tau_j}\mathcal{B}_{j, m-1}(x)  + \frac{\tau_{j + m} - x}{\tau_{j + m } - \tau_{j +1}}\mathcal{B}_{j + 1, m-1}(x),
$$
for $j=1,\dots,K + 2M - m$ and for $m=1,\dots,M$. 

## ☠️ - Properties of B-splines

- B-splines have [local support]{.orange}, which means that for $j=1,\dots,K + 2M - m$ we have
$$
\begin{aligned}
&\mathcal{B}_{j, M}(x) = 0, \qquad x \not\in (\tau_j, \tau_{j+M}), \\
&\mathcal{B}_{j, M}(x) > 0, \qquad x \in (\tau_j, \tau_{j+M}).
\end{aligned}
$$
This implies that the support of cubic B-splines is at most $5$ knots. 

. . .

- The [intercept]{.blue} term is [implicitly included]{.orange} in a B-spline basis, in fact 
$$
\sum_{j=1}^{K + M}\mathcal{B}_{j, M}(x) = 1, \qquad x \in (a, b).
$$
