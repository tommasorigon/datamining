...
$$
\hat{f}(x) = \frac{1}{\sum_{i'=1}^n w_{i'}(x)}\sum_{i=1}^n w_i(x) y_i = \sum_{i=1}^n \tilde{w}_i(x) y_i,
$$
where $\tilde{w}_i(x) = w_i(x) / \sum_{i'=1}^n w_{i'}(x)$ are the [normalized weights]{.blue}.
...
- If $f(x)$ is differentiable in $x_0$, then it can be locally approximated with a linear function tangent in $x_0$, that is
$$
...
- Hence, instead of computing a [local average]{.orange} as before, we consider a [local linear model]{.blue}. In other words, for every $x$ we seek the coefficients solving:
$$
(\hat{\beta}_1, \hat{\beta}_2) = \arg\min_{(\beta_1, \beta_2)} \sum_{i=1}^n w_i(x)\left\{y_i - \beta_1 - \beta_2(x_i - x)\right\}^2.
$$
Then, the local [linear regression estimator]{.blue} for the point $x$ is  equal to 
$$
...
- The local linear regression optimization problem, as we have seen in [Unit A](un_A.html), has an [explicit solution]{.orange}:
$$
