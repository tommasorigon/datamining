...
-   There are [no]{.orange} specific reasons to believe that
    $f(x) = \alpha + \beta\log{x}$ or any other [parametric
    function]{.orange}.
...
-   Indeed, in a typical datasetit is very unlikely that there exist
...
-   Instead of considering the values exactly equal to $x$, what we
    could do is select the pairs $(x_i, y_i)$ that are [close]{.blue} to
    (in a [neighbour]{.orange} of) $x$.

-   A natural measure of proximity between $x$ and the data points $x_i$
    is the [Euclidean distance]{.orange} $|x_i - x|$, but in principle
    any other metric could be considered.

. . .

-   We consider an average of the $k$ values $y_i$ whose $x_i$ are
    closest to $x$, that is: $$
    \hat{f}(x) = \frac{1}{|\mathcal{N}_x|}\sum_{i \in \mathcal{N}_x} y_i,
    $$ where $\mathcal{N}_x$ is the set of $k$ points nearest to $x$ in
    Euclidean distance.
...
-   Indeed, as we move $x$ from left to right, the $k$-nearest
    neighborhood remains constant, until a point $x_i$ to the right of
    $x$.
...
- [Bounded]{.blue} kernels have [computational]{.blue} advantages, because one need to compute averages of a limited number of data points. 
...
- Intriguing idea: what about selecting $h$ so that the [asymptotic mean squared error]{.blue} is [minimal]{.orange}? This leads to the following optimal choice for the bandwidth: $$
