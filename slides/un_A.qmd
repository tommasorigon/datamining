...
$\text{rk}(\bm{X}) = p$, then the [least square
...
-   $\bm{H}$ is a $n \times n$ [projection matrix]{.orange}
...
-   The quantity $D(\hat{\beta})$ is the so-called residual [deviance]{.blue},
    which is equal to $$
...
-   Confidence intervals and Wald's tests can be obtained through
...
-   The overall fit [seems satisfactory]{.blue} at first glance,
    ...
    the engine size is average (i.e., between $1.5L$ and $3L$).
...
-   In this third model, we handled the [outliers]{.orange} appearing in
    the residual plots, which turn out to be identified by the group
...
-   The first part of the unit is a tour of the "old style" data
    modeling, the kind of culture that Leo Breiman so [heavily
    criticized]{.orange} in his 2001 *Statistical Science* paper.

. . .

-   However, this dataset was sufficiently small, meaning it could be
    "manually" analyzed and modeled. We gained much
    [understanding]{.blue} by doing so.

-   Hence, these old tools should not be considered useless or
    irrelevant.

. . .

-   The second half of the unit will have an entirely different flavor,
    though.

-   Given the vast amount of data we now have, it makes sense to focus
    on [computations]{.orange} for fitting linear models.

-   As we will see, the mathematical simplicity of linear models leads
    to extremely [fast computations]{.blue}, an important advantage in
    the era of big data.
...
-   Recall (from your favorite linear algebra textbook) that a
    ...
    -   The quadratic form $\bm{x}^T \bm{A} \bm{x} > 0$ for all
        $\bm{x} \in \mathbb{R}^p$ such that $\bm{x} \neq 0$.
...
-   Let $\bm{R}^T\bm{R}$ be the Cholesky factorization of the matrix
    ...
    $$ This system can now be solved in [two steps]{.orange}:
...
::: incremental
-   Clearly, the predictors in $\bm{X}$ are generally not orthogonal.
    Hence, we want to find a suitable transformation
    $\bm{Z} = \bm{X} \bm{\Gamma}^{-1}$ that [orthogonalizes]{.blue} the
    [predictors]{.orange}.
...
-   It is often convenient to [standardize]{.orange} the columns of
    ...
    \bm{X} = \bm{Z} \bm{\Gamma} = \bm{Z} \bm{D}^{-1} \bm{D} \bm{\Gamma} = \bm{Q} \bm{R}, \quad \text{with} \quad \bm{Q} = \bm{Z}\bm{D}^{-1}\quad \text{and} \quad \bm{R} = \bm{D}\bm{\Gamma}.
    $$
...
-   The default approach in R, i.e., the one implemented in the `lm`
    function is the QR factorization because one typically also needs to
    compute $\bm{H}$, or the leverages.
:::
...
-   If $\text{rk}(\bm{X}) = k < p$ ([rank deficiency]{.orange}) then it
    ...
    0 & 0\end{bmatrix},
    $$ where $\bm{P}$ is a $p \times p$ permutation matrix and $\bm{R}_{11}$
...
-   In the presence of perfect collinearity, the implementation of the
    QR decomposition in R (`qr`) relies on pivoting. This is why the
    `lm` function can automatically "omit" a predictor.
:::
...
::: incremental
-   On many occasions, the data flows continuously, meaning that we get
    a new pair of observations $(\bm{x}_{n+1}, y_{n+1})$ every minute,
...
-   The recursive estimation
    $\hat{\beta}_{(n+1)} = \hat{\beta}_{(n)} + \bm{k}_n e_{n+1}$ takes
    the form of a [linear filter]{.orange}, in which the new estimate
...
-   Let us consider the South African `heart`
    ...
    described in [Section 4.4.2]{.blue} of HTF (2009).
...
-   We want to [predict]{.blue} the insurgence of [coronary heart
    ...
    -   Consumption of tobacco (`tobacco`) and alcohol (`alcohol`)
...
-   We let $Y_i$ be iid draws from an [exponential dispersion
...
-   The canonical statistical solution are [generalized linear
    ...
    (e.g. [Statistica III](https://tommasorigon.github.io/StatIII/)). This is just a [short recap]{.orange}.
...
\mathbb{E}(Y_i) = g^{-1}\{f(\bm{x}_i; \beta)\} = g^{-1}(\beta_1 x_{i1} + \cdots + \beta_p x_{ip}) = g^{-1}(\bm{x}_i^T\beta),
$$ where $g(\cdot)$ is the so-called [link function]{.orange}, which is known and invertible.
:::
...
-   When $Y_i$ are Gaussian random variables, then the log-likelihood is
    $$
    \ell(\beta, \sigma^2) = -\frac{n}{2}\log{\sigma^2} - \frac{1}{2\sigma^2}D(\beta),
...
-   Note in addition that the log-likelihood, evaluated at its maximum,
    is $$
    -2 \ell(\hat{\beta},\hat{\sigma}^2) =  n \log\{D(\hat{\beta})/n\} + n,
...
-   The [canonical link]{.blue} function $g^{-1}(x) = e^x / (1 + e^{x})$
    leads to the GLM $$
...
-   Once again, the Cholesky and the QR decomposition can be exploited
    to speed up computations. See Section 5.4 of Arnold et al. (2019)
    for further details.
...
-   The coefficient of the variable `obesity` is [negative]{.orange}.
...
-   Hence, the [overall accuracy]{.orange} is
    $(255 + 82) / 462 \approx 0.72$.

. . .

-   The [true negative rate]{.blue} ([specificity]{.orange}) is defined
    as $$
    \text{specificity} = 1 - \mathbb{P}(``\text{false positive}") \approx \frac{n_{00}}{n_{00} + n_{10}} = \frac{255}{255 + 47} = 0.844.
    $$
-   On the other hand, the [true positive rate]{.blue}
    ([sensitivity]{.orange}) is defined as $$
...
-   When the number of responses labels $K > 2$, we cannot use any more
    logistic regression.

-   The natural extension is called [multinomial regression]{.orange},
    in which we model the probabilities $$
    \mathbb{P}(Y_i = k) = p_k(\bm{x}_i,\beta), \qquad k=1,\dots,K,
    $$ for example, using [multinomial logit]{.orange}. The ideas of
    GLMs can be easily borrowed.

-   An alternative and straightforward approach is [linear discriminant
    analysis]{.blue}, which is based on Bayes theorem.

-   You can find these topics in the textbook A&S (2011), although you
    have likely seen these models in the previous courses.

## References

-   [Main references (M.Sc. level)]{.blue}
    -   **Chapters 2 and 5** of Azzalini, A. and Scarpa, B. (2011),
        [*Data Analysis and Data
        Mining*](http://azzalini.stat.unipd.it/Book-DM/), Oxford
        University Press.
    -   **Chapters 3 and 4** of Hastie, T., Tibshirani, R. and
