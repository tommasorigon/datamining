...
-   Also, this model in not suitable for [extrapolation]{.orange}.
    ...
    -   It has no grounding in physics or engineering, which leads to
        difficulties in the interpretation of the trend and/or
...
-   A major advantage of linear models is that they can describe
    non-linear relationship via [variable transformations]{.blue} such
...
-   Yet, there is still some heteroschedasticity in the residuals --- is
    this due to a missing covariate that has not been included into the
    model?
...
-   In this third model, we took care of the [outliers]{.orange}
...
- The first part of the unit is a tour in the "old style" data modelling, the kind of culture that Leo Breiman so [heavily criticized]{.orange} in his 2001 *Statistical Science* paper.

- However, this dataset was sufficiently small, meaning that it could be "manually" analyzed and modeled. We gained a lot of [understanding]{.blue} by doing so. 
...
- Given the huge amount of data we now have, it makes a lot of sense to focus on [computations]{.orange} for fitting linear models.
...
-   Recall (from your favorite linear algebra textbook) that a
    ...
    -   The quadratic form $\tilde{\bm{x}}^T \bm{A} \tilde{\bm{x}} > 0$
        for all $\tilde{\bm{x}} \in \mathbb{R}^p$ such that
        $\tilde{\bm{x}} \neq 0$.
...
-   We want to [predict]{.blue} the insurgence of [coronary heart
    disease]{.orange} (`chd`) as function of known risks factors:

    -   Colesterol level (`ldl`) and obesity (`obesity`)
    -   Consumption of tobacco (`tobacco`) and alcool (`alcool`)
...
-   If the outcome is binary, a count, or if the errors are
    heteroschedastic, then the Gaussian linear regression model might be
    inappropriate.
...
-   The canonical statistical solution are [generalized linear
    models]{.orange}, which are usually thought in undergraduate courses
    (e.g. Statistica III). This is just a short recap.
...
$$ where $g(\cdot)$ is the so-called [link function]{.orange}, which is
...
-   In [matrix notation]{.orange}, we will write $$
    ...
    and $W = \text{diag}(w_1(\beta),\dots, w_n(\beta))$.
...
-   Unfortunately, the IRLS is [not]{.orange} garanteed to
...
- An alternative and simple approach is [linear discriminant analysis]{.blue}, which is based on Bayes theorem. 
...
-   [Basic references (B.Sc. level)]{.grey}
    -   **Chapters 5 and 6** of A. Azzalini (2000), [*Inferenza
