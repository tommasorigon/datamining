...
-   Let us consider the South African `heart`  [dataset](https://hastie.su.domains/ElemStatLearn/datasets/SAheart.data), described is [Section 4.4.2]{.blue} of HTF (2009).

-   We want to [predict]{.blue} the insurgence of [coronary heart disease]{.orange} (`chd`) as function of known risks factors:
...
- The response variable is [binary]{.orange}: we cannot rely on linear regression models.
:::
:::
...
<!-- p0 <- ggpairs(heart, -->
<!--   columns = c(1:4, 6:9), aes(colour = chd), -->
...
- Hence, we let $Y_i$ be iid draws from an [exponentialy dispersion family]{.blue}, which includes  the Binomial, the Poisson, and the Gaussian distribution as special cases.

- The canonical statistical solution are [generalized linear models]{.orange}, which are usually thought in undergraduate courses (e.g. Statistica III). This is just a short recap.
...
In a [generalized linear model]{.blue} the response variable is related to the covariates through:
$$
...
-   [Basic references (B.Sc. level)]{.gray}
    -   **Chapters 5 and 6** of A. Azzalini (2000), [*Inferenza Statistica. Una presentazione basata sul concetto di verosimiglianza*](http://azzalini.stat.unipd.it/Book-DM/), Springer.
    -   **Chapter 3** of Hastie, T., Tibshirani, R. and Friedman, J.
        (2009), [*The Elements of Statistical
        Learning*](https://hastie.su.domains/ElemStatLearn/), Second
        Edition, Springer.
-   [Advanced references (Ph.D. level)]{.orange}
