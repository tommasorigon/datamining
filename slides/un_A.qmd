...
-   However, the plot of the [residuals]{.orange}
    $r_i = y_i - \hat{y}_i$ suggests that there the homoscedasticity
...
-   The [updated least square estimate]{.orange} therefore becomes $$
    ...
    &=\hat{\beta}_{(n)} +  \underbrace{v_{(n)} \bm{V}_{(n)}\bm{x}_{(n + 1)}}_{\bm{k}_n}\underbrace{(y_{n+1} - \bm{x}_{(n + 1)}^T\hat{\beta}_{(n)})}_{e_{n + 1}} \\
