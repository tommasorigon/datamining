...
- At the cost of being repetitive: especially when data are complex, you should first check whether a simple solution exists. Sometimes this is actually the case, because "simple" models tend to have low variance. 
...
- Other [details]{.orange}: is it preferable to perform the merging before estimating the model (using all the observations for fitting it), or after the estimation procedure (based on a reduced dataset), when making the predictions? This is just another instance of the [bias-variance trade-off]{.orange}. Hence, there are no general answers. 
