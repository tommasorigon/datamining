...
- I will collect here [interesting]{.blue} questions asked by students over the years.
...
- Indeed, it draws on the knowledge of several statisticians. 
:::
:::
...
- Whenever different estimates display some differences, try to see it as an [opportunity]{.blue} to [learn something from the data]{.orange}. What could be the reason for such a discrepancy? What does this tell us about the data we are analyzing?
:::
...
- In that case, of course, it is time to move on and try something more sophisticated. This often occurs in practice; only a few case-studies can be solved with simple tools.
...
- This question was not asked during an M.Sc. level course such as Data Mining at CLAMSES (otherwise, it would have been a bit depressing), but it happened in [other contexts]{.orange}.
:::
...
- Other [details]{.orange}: is it preferable to perform the merging before estimating the model (using all the observations for fitting it) or after fitting the model on the reduced training set, when making predictions? This is another instance of the [bias-variance trade-off]{.orange}. Hence, there are no general answers. 
