...
- The following illustration may help in clarifying this notion of [sparsity]{.orange}. Let us consider data points that are uniformaly distributed on $(0, 1)^p$, that is $\bm{x}_i \overset{\text{iid}}{\sim} \text{U}^p(0, 1)$.
...
$$
Note that even with a much larger sample size, we get $\text{dist}(10, 1000) = 0.483$. 

- The vast majority of points is [close to the boundary]{.blue}, making predictions very hard.
:::
...
- Another [intuition]{.orange} of the curse of dimensionality is given by the following argument. Let us consider again [uniform]{.blue} covariates $\bm{x}_i \overset{\text{iid}}{\sim} \text{U}^p(0, 1)$. 
...
- In the more general case, in can be shown that, on [average]{.orange}, the [side]{.blue} of the cube is
$$
\text{side}(r, p) = r^{1/p},
$$
...
- If $f(x)$ is assumed to be [arbitrarily complex]{.orange} and our estimator $f(x)$ is [nonparametric]{.blue}, we destined to face the curse.
...
- Nature is not necessarily a linear model, this is why we wanted to go explore the nonparametric case.

- Nonetheless, making (correct) [assumptions]{.orange} and therefore imposing (appropriate) restrictions is [beneficial]{.blue}, to the extent that is [unavoidable]{.orange} in high-dimensions. 
:::
...
- These [assumptions]{.orange} differentiate because of:
    - The particular [nature]{.grey} of the knowledge the impose (e.g. no interactions, piecewise constant functions, etc.);
    - The [strength]{.orange} of this assumption;
    - The [sensibility]{.blue} of the methods to potential violation of the assumptions.
...
- ...or at least, she/he will be aware of the [limitations]{.orange} of the methods. 
:::