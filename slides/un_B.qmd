...
-   Akaike suggested minimizing over $p$ the following [Kullback-Leibler
    divergence]{.orange}: $$
...
-   In the above Kullback-Leibler, for any $p$ the value $\theta$ is replaced by the [maximum likelihood estimator]{.blue} $\hat{\theta} = \hat{\theta}(\bm{Y})$, using the data $\bm{Y} = (Y_1,\dots,Y_n)$.
    
- Hence, we would like to select $p$ such that the expectation w.r.t. $p(\bm{Y}; \theta_0)$ 
$$
\begin{aligned}
...
\end{aligned}
$$
is [minimized]{.orange}. However, we cannot compute nor minimize $S(p)$, because $\theta_0$ is unknown. 
...
- Although $S(p)$ cannot be obtained, under technical conditions it turns out that
$$
...
- The factor $2$ is just a [cosmetic convention]{.blue}, which has been introduced to match the quantities appearing in the usual asymptotic theory. 
