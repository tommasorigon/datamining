...
- In many books, including A\&S, the starting point of the analysis is the [reducible error]{.blue}, because it is the only one we can change and minimize, e.g. by selecting the "optimal" $p$.

- It measures the [discrepancy]{.orange} between the unknown function $f(x)$ and its estimate $\hat{f}(\bm{x})$.
...
- Summarizing, the [expected prediction error]{.orange} $\text{Err}$ can be decomposed as
$$
\begin{aligned}
...
- The [bias]{.orange} measures the ability of the estimator $\hat{f}(\bm{x})$ to reconstruct the true $f(\bm{x})$. It equals zero whenever $\mathbb{E}\{\hat{f}(\bm{x})\} = f(\bm{x})$. 
