...
-   Polynomial regression becomes numerically unstable when $p \ge 13$
    (`raw = TRUE`, original polynomials) and $p \ge 25$
    (`raw = FALSE`, orthogonal polynomials).
...
-   In [ordinary least squares]{.blue} the above quantity can be
    ...
    $$ where $\bm{f} = (f(\bm{x}_1),\dots,f(\bm{x}_n))^T$. Note that [if]{.orange}
    $f(\bm{x}) = \bm{x}^T\beta$, then the bias is zero.

-   Moreover, in [ordinary least squares]{.blue} the [variance]{.orange}
    term equals $$
    \frac{1}{n}\sum_{i=1}^n\text{var}\{f(\bm{x}_i; \hat{\beta})\} = \frac{\sigma^2}{n}\sum_{i=1}^n  \bm{x}_i^T (\bm{X}^T\bm{X})^{-1}\bm{x}_i = \frac{\sigma^2}{n}\text{tr}\{(\bm{X}^T\bm{X})(\bm{X}^T\bm{X})^{-1}\} = \sigma^2 \frac{p}{n}.
    $$
:::
...
-   If [ordinary least squares]{.blue} are employed, then the predictions are $\bm{H}\bm{Y}$, therefore $$
...
::: incremental
-   An alternative to LOO-CV is the so-called [generalized cross validation]{.orange} (GCV), defined as $$
    \text{GCV} = \widehat{\mathrm{Err}} = \frac{1}{n}\sum_{i=1}^n \left(\frac{y_i - \hat{y}_i}{1 - p/n}\right)^2.
    $$
-   The GCV is an approximate LOO-CV for [ordinary least squares]{.blue}, in which the leverages $h_i$ are
