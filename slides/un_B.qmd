...
-   Examples of loss functions for [binary classification
    ...
    -   The [misclassification loss]{.orange}, which is defined as $$
        ...
        $$ The predictions are obtained dichotomizing the probabilities
        $\hat{y}_i = \mathbb{I}(\hat{p}(\bm{x}_i) > 1/2)$.
...
-   Indeed, the estimate $\widehat{\text{se}}$ of the standard deviation
    ...
    $r_1, \dots, r_n$ are independent, but this is not true!
:::
...
-   In the `cholesterol` dataset, the various indices produced
    ...
    -   The BIC and and the $10-$fold cross-validation selected $p = 2$
        (linear model);
    -   The the training / test split suggested $p = 3$ (quadratic
        model);
...
-   The good news is that all the above methods produced [similar
    findings]{.blue}. For example, we are quite sure that we should
    choose $p \le 6$.

-   On the other hand, there is obviously some [uncertainty]{.orange},
