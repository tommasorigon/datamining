...
-   Since we do not know $f(\bm{x}_i)$, we seek an estimate
...
## Bias-variance in linear regression models 📖
...
::: incremental
-   It can be proved that the [optimism]{.orange} has a
    very simple form: $$
...
-   The error on each of the $k$th folds is computed as
...
-   A $K$-fold CV with $K=5$ or $K = 10$ is an upward-[biased
...
-   The main statistical method for estimating unknown parameters of a
    model is to [maximize the log-likelihood]{.blue}
...
-   The $\text{AIC}_c$ is a [higher order correction]{.orange} of the
    $\text{AIC}$ and the differences tend to be negligible for high
    values of $n$.

-   The justification of $\text{BIC}$ comes from [Bayesian
    statistics]{.blue}.
...
-   In the `cholesterol` dataset, the various indices produced
    ...
    -   The BIC and the $10-$fold cross-validation selected $p = 2$
        (linear model);
