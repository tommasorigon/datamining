...
-   [Remark]{.orange}. Tomorrow's r.v. $\tilde{Y}_1,\dots, \tilde{Y}_n$
    follow the same scheme of yesterday's data.
:::
...
-   Let us better formalize the analysis we did so far, using the appropriate
    mathematical tools.
...
-   The [training data]{.orange} is used to estimate a function of the
    covariates $\hat{f}(\bm{x}_i)$. We hope that our predictions works well on the [test set]{.blue}.
...
- The principles described for regression settings also applies to [classification problems]{.blue}, after the appropriate adjustments. We focus here on the binary case $Y \in \{0, 1\}$.

- In this context, the relationship between $\bm{x}_i$ and the [Bernoulli]{.orange} r.v. $Y_i$ is such that 
$$
...
-   Examples of loss functions for [binary classification problems]{.blue}
    $Y \in \{0, 1\}$ are:
    -   The [misclassification rate]{.orange}
        $\mathscr{L}\{\tilde{Y}_i; \hat{f}(\bm{x}_i)\} = \mathbb{I}(\tilde{Y}_i  \neq \hat{y}_i)$,
        where the predictions are obtained dichotomizing the estimated probabilities $\hat{y}_i = \mathbb{I}(\hat{p}(\bm{x}_i) > 1/2)$.
    -   The [deviance]{.orange} or [cross-entropy]{.orange} loss functions is defined as
        $$
        \mathscr{L}\{\tilde{Y}_i; \hat{f}(\bm{x}_i)\} = -2\left[ \mathbb{I}(Y_i = 1)\log{\hat{p}(\bm{x}_i)} + \mathbb{I}(Y_i = 0)\log{\{1 - \hat{p}(\bm{x}_i)}\}\right].
        $$
- Under a misclassification loss, the [Bayes rate]{.blue} is the error rate we would get if we knew the true $p(\bm{x}_i)$. This is the [irreducible error]{.orange} for classification problems.  
...
-   In many textbooks, including A&S, the starting point of the analysis
    ...
    control and it has a transparent interpretation.
...
-   We can [randomly]{.orange} split the original set data
...
-   We must take into account the different number of parameters,
...
-   For any $p$, Akaike suggested minimizing the so-called [Kullback-Leibler
    divergence]{.orange}: $$
    ...
    $$ between the "true" model $p(\bm{Y};\theta_0)$ with parameter $\theta_0$ and the specified model $p(\bm{Y};\theta)$.

-   We evaluate this discrepancy in the value of $\theta$ that maximizes $\log{p(\tilde{\bm{Y}};\theta)}$ using the data $\bm{Y} = (Y_1,\dots,Y_n)$, namely in the [maximum likelihood estimator]{.blue} $\hat{\theta} = \hat{\theta}(\bm{Y})$.
    
- The Akaike (ideal) approach would select $p$ such that the expectation w.r.t. $p(\bm{Y}; \theta_0)$ 
$$
\begin{aligned}
S(p) &= 2 \: \mathbb{E}_{\theta_0}\left[\text{KL}(p(\cdot; \theta_0) \mid\mid p(\cdot;\hat{\theta}))\right] - 2 \int p(\tilde{\bm{Y}};\theta_0) \log{p(\tilde{\bm{Y}};\theta_0)}\mathrm{d}\tilde{\bm{Y}} \\
& = - 2 \: \mathbb{E}_{\theta_0}\left[\int p(\tilde{\bm{Y}};\theta_0)\log{p(\tilde{\bm{Y}};\hat{\theta})\mathrm{d}\tilde{\bm{Y}}} \right] 
\end{aligned}
$$
is [minimized]{.orange}. Unfortunately, we cannot compute $S(p)$, because $\theta_0$ is unknown. 

:::

## The Akaike information criterion II

- Although $S(p)$ cannot be obtained, under several technical conditions it turns out that
$$
\text{AIC} = -2 \ell(\hat{\theta}) + 2 p,
$$
namely the [Akaike information criterion]{.orange} is a good estimator of $S(p)$.

- More formally, it can be proved (under technical conditions) that
$$
\mathbb{E}_{\theta_0}(\text{AIC}) + o(1) = S(p),
$$
for $n \rightarrow \infty$. 

- The factor $2$ is just a [convention]{.blue} and is introduced to match the quantities appearing in the usual asymptotic theory for log-likelihoods. 

## AIC, AICc, BIC

-   Akaike's original work was followed by several other proposals,
    different in their assumptions and the way they approximate certain
    quantities.
...
-   The $\text{AIC}_c$ is an [higher order correction]{.orange} of the
    $\text{AIC}$ and the differences tends to be negligible for high
    values of $n$.
...
-   Since $\log{n} > 2$ for any $n > 7$, it means that the $\text{BIC}$
    [penalty]{.blue} is typically [stronger]{.blue} than the one of
    $\text{AIC}$.
