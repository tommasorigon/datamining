...
- The example with yesterday's and tomorrow's data can be better formalized.

- In general settings, we consider two datasets of [iid random variables]{.blue}: 
  - [Training]{.orange} $(X_1, Y_1),\dots,(X_n, Y_n)$, whose realization is $(\tilde{\bm{x}}_1, y_1), \dots, (\tilde{\bm{x}}_n, y_n)$.
  - [Test]{.blue} $(X_{n+1}, Y_{n+1}),\dots,(X_{n +m}, Y_{n +m})$, with realization $(\tilde{\bm{x}}_{n+1}, y_{n+1}), \dots, (\tilde{\bm{x}}_{n +m}, y_{n + m})$.
- The [key assumption]{.orange} behind most statistical learning algorithm is that the random vectors  $$(X_i,Y_i) \overset{\text{iid}}{\sim} \mathcal{P}, \qquad i=1,\dots,m,$$ 
where $X_i \in \mathbb{R}^p$ represents a vector of [covariates]{.blue} and $Y_i$ is the [response]{.orange}.

- [Step 1]{.blue}. Use the [training data]{.orange} to estimate a function of the covariates $\hat{f}(X_i)$ which should predict well the corresponding response $Y_i$.

- [Step 2]{.blue}. Evaluate the quality of our predictions by measuring the discrepancy between $\hat{f}(X_i)$ and $Y_i$, using the [test set]{.orange}.
...
- A common measure of discrepacy between $\hat{f}(X_i)$ and $Y_i$, based on the test set, is: $$
\text{Err}_\text{test} = \frac{1}{m} \sum_{i=1}^m \mathscr{L}\{Y_{n+i}; \hat{f}(X_{n+i})\},
$$
where $\mathscr{L}\{Y_i; \hat{f}(X_i)\}$ is a [loss function]{.orange}.

- Examples of loss functions for [regression problems]{.blue} $Y_i \in \mathbb{R}$ are: 
  - The quadratic loss $\mathscr{L}\{Y_i; \hat{f}(X_i)\} = \{Y_i - \hat{f}(X_i)\}^2$.
  - The absolute loss $\mathscr{L}\{Y_i; \hat{f}(X_i)\} = |Y_i - \hat{f}(X_i)|$.

- In practice, it is convenient to consider the so-called [expected prediction error]{.blue}
$$
\text{Err} = \mathbb{E}(\text{Err}_\text{test} ) = \frac{1}{m} \sum_{i=1}^m \mathbb{E}\left[\mathscr{L}\{Y_{n+i}; \hat{f}(X_{n+i})\}\right] = \mathbb{E}\left[\mathscr{L}\{Y_{n+1}; \hat{f}(X_{n+1})\}\right],
$$
where the expectation is taken with respect to both the training and the test set. 
:::
...
- We assume that the [covariates]{.blue} $\tilde{\bm{x}_i}$ are [deterministic]{.blue} (fixed-X design). If they were not, one just needs to include the conditioning $(X_{n+1} = \tilde{\bm{x}}_{n+1})$ in all the following expectations.

- The expected prediction error, under a [quadratic loss]{.orange} and fixed-X, becomes
$$
\text{Err} = \mathbb{E}(\text{MSE}_\text{test} ) =\mathbb{E}\left[\{Y_{n+1} - \hat{f}(\tilde{\bm{x}}_{n+1})\}^2\right].
$$
