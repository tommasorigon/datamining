...
-   MARS combine many of the techniques we have seen into a single
...
-   The main distinction with GAMs is that in MARS the basis functions
    ...
    are not pre-specified in advance.
...
-   The knots are placed in correspondence of the observed data. Hence,
...
-   In the subsequent step, we consider a [new pair]{.blue} of basis
    ...
    ii. We can include the new pair in a [multiplicative]{.orange} way,
        ...
        one of the [old basis]{.blue} of the model, obtaining for
        instance $$
           \begin{aligned}
...
2.  Identify the [initial pair]{.blue} of basis functions $h_1$ and
    $h_2$ in $\mathcal{C}$ that minimize the mean squared error. Then,
...
3.  For $k = 1,\dots,K - 1$, do:
    ...
    iv. Identify the [optimal pair]{.blue} among the candidates at steps
        (ii)-(iii) that [reduces]{.orange} the [mean squared
        error]{.orange} the most. This results in a new pair of bases
        $h_{2k+1}, h_{2k+2}$.
...
-   The final model with $M$ terms is very likely [overfitting]{.orange}
    the data. Hence, it is common practice to remove some of the basis
...
-   Unfortunately, here it is not entirely clear what could be the
    appropriate number of [effective degrees of freedom]{.orange} to be
    used in the $\text{GCV}$.
...
-   When the basis functions in $\mathcal{C}$ are multiplied together,
    the result is nonzero only over the small part of the feature space
    where both component functions are nonzero.

. . .

-   Hence, the estimated function is built up [parsimoniously]{.blue},
    by making [small local modifications]{.orange} to the previous fit.

-   This is important, since one should "spend" parameters carefully in
    high dimensions, to avoid incurring into the [curse of
    dimensionality]{.orange}.
