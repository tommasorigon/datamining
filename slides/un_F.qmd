...
-   Such an approach is particularly convenient in combination with
    [smoothing splines]{.orange}, because the [derivatives]{.orange} needed for Newton's
    method are available in closed form.
...
-   Let us get back to the `trawl` data. A [specification]{.blue} based on GAM could
    be$$
      \begin{aligned}
...
|:---------------|---------:|----------:|------:|
| `(Intercept)`    |    0.865 |     0.099 |     1 |
| `s(Longitude)` |       \- |        \- | 4.427 |
| `s(Latitude)`  |       \- |        \- | 3.187 |
| `s(Depth)`     |       \- |        \- | 1.580 |
| `Zone_Closed`  |   -0.265 |     0.137 |     1 |
| `Year_1993`    |    0.300 |     0.081 |     1 |
:::
...
- Recall that the [naïve Bayes classifier] expresses the classification probability $\text{pr}(y = \ell \mid \bm{x})$ as
$$
\text{pr}(y = \ell \mid \bm{x}) = \frac{\pi_\ell\prod_{j=1}^p p_{j\ell}(x_j)}{\sum_{m = 1}^L \pi_m\prod_{j=1}^p p_{jm}(x_j)} = \frac{\pi_\ell\prod_{j=1}^p p_{j\ell}(x_j)}{p(\bm{x})}, \quad \ell = 1,\dots,L.
$$
...
::: {.column width="65%"}
- GAMs were invented by Hastie and Tibshirani in 1986
:::
:::
...
- Sentence from Hastie about interpretability

- Easy to fit

- Potential manual insertion of interaction
:::
