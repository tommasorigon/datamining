...
# Data import
train <- read.csv("../data/titanic_tr.csv", header = TRUE, stringsAsFactors = FALSE)
n <- nrow(train)

test <- read.csv("../data/titanic_tr.csv", header = TRUE, stringsAsFactors = FALSE)
m <- nrow(test)
...
boxplot(price ~ pclass + embarked + sex, data = combi, subset = pclass == 1 & sex == "female")
abline(h = 40)
